...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        self.fine_width=192
        self.radius=5
        self.dataset_size = len(open('train_pairs.txt').readlines())
    ...
    def __getitem__(self, index):        
        ...
        pose_name =I_path.replace('.png', '_keypoints.json').replace('.jpg','_keypoints.json').replace('train_img','train_pose')
        ...
        densepose_name = I_path.replace('.png', '.npy').replace('.jpg','.npy').replace('train_img','train_densepose')
        dense_mask = np.load(densepose_name).astype(np.float32)
        dense_mask = Image.fromarray(np.uint8(dense_mask))
        dense_mask = transform_A(dense_mask) * 255.0

        input_dict = { 'image': I_tensor,'clothes': C_tensor, 'edge': E_tensor, 'label': A_tensor, 'pose':P_tensor,'densepose': dense_mask}
        return input_dict
